...
namespace QuanLyQuanCafe
{
    public partial class fTableManager : Form
    {
        ...
        void ShowBill(int id)
        {
            ...
            //Cách 1 đổi về đơn vị VNĐ
            #region 
            //Thread.CurrentThread.CurrentCulture = culture;
            //txbTotalPrice.Text = totalPrice.ToString("c");
            #endregion

            //Cách 2 đổi về đơn vị VNĐ
        ...
        private void btnCheckOut_Click(object sender, EventArgs e)
        {
            ...
            if (idBill != -1)
            {
                if (MessageBox.Show(string.Format("Bạn có chắc thanh toán hóa đơn cho bàn {0}\nTổng tiền - (Tổng tiền / 100) * Giảm giá\n=> {1}k - ({1} / 100) x {2}% = {3}k", table.Name, totalPrice, discount, finalTotalPrice), "Thanh toán", MessageBoxButtons.OKCancel) == System.Windows.Forms.DialogResult.OK)
                {
